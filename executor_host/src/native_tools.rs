...
impl NativeToolExecutor {
    ...
    fn scan_directory(path: &Path, tools: &mut HashMap<String, LoadedNativeTool>) -> Result<()> {
        if !path.exists() || !path.is_dir() {
            return Ok(());
        }
            ...
            if dir.is_dir() {
                ...
                if manifest_path.exists() {
                    if let Ok(content) = std::fs::read_to_string(&manifest_path) {
                        if let Ok(manifest) = serde_json::from_str::<NativeToolManifest>(&content) {
                ...
                // Recursively scan subdirectories (e.g. skills/data_analyst/compute_statistics)
                let _ = Self::scan_directory(&dir, tools);
            }
        }
        Ok(())
    }
