...
use std::sync::Arc;
use trust_core::envelope::{ExecutionResult, GrantedAction, TrustEnvelope};
...
impl Runtime {
    pub async fn new(
        ...
        grant_validator: Arc<GrantValidator>,
    ) -> Result<Self> {
        ...
        let execution_results_kv = match js
    ...
    pub async fn run(&self, executor: Arc<dyn Executor>, profile: &str) -> Result<()> {
            ...
            .await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "⚠️ EXEC_STREAM stream creation failed, trying to bind to existing: {}",
                    e
                );
        ...
        let subject_filter = format!("exec.v1.*.{profile}.invoke");

        // Delete existing consumer if it exists so that updated config (e.g. ack_wait) takes effect
        if let Ok(mut old_consumer) = stream
            .get_consumer::<async_nats::jetstream::consumer::pull::Config>(&consumer_name)
            .await
        {
            if let Ok(info) = old_consumer.info().await {
                if info.config.ack_wait != std::time::Duration::from_secs(120) {
                    tracing::info!("🔄 Deleting old consumer {} (ack_wait={:?}) to recreate with updated config", consumer_name, info.config.ack_wait);
            ...
            .await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "⚠️ Consumer {} creation failed, trying to bind to existing: {}",
                    consumer_name,
                    e
                );
                stream
                ...
                Ok(mut messages) => {
                    while let Some(msg_result) = messages.next().await {
                        match msg_result {
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "⚠️ Message stream error: {} — reconnecting consumer...",
                                    e
                                );
                                break;
                            }
    ...
    async fn handle_invocation_jetstream(
        ...
        tracing::info!(
            "📥 [{}] Handling JetStream invocation for tool: {}",
            action_id,
            ...
            .await
        {
            Ok(g) => g,
            Err(e) => {
                let err_res = self.create_error_envelope(
                    &envelope,
                    format!("Grant validation failed: {e}"),
                    profile,
                );
                self.nats
                    .publish(
                        envelope.payload.reply_subject.clone(),
                        serde_json::to_vec(&err_res)?.into(),
                    )
                    .await?;
                let _ = msg.ack().await;
                return Ok(());
        ...
        let result = match executor
            .execute(
                VerifiedGrant::new(grant),
                envelope.payload.canonical_args.clone(),
            )
            .await
        {
            Ok(output) => {
        ...
        self.execution_results_kv
            ...
            .await?;
        self.nats
            .publish(envelope.payload.reply_subject.clone(), payload.into())
            .await?;
        ...
        tracing::info!(
            "✅ [{}] JetStream Execution completed and result published",
            action_id
        );
        Ok(())
    }
    ...
    fn create_error_envelope(
        &self,
        req: &TrustEnvelope<GrantedAction>,
        error: String,
        profile: &str,
    ) -> TrustEnvelope<ExecutionResult> {
        TrustEnvelope {
            schema_version: 1,
