...
impl JetStreamAgentRegistry {
    ...
    async fn index_source(&self, agent_id: &str, agent_type: AgentType, policy_profile: &str) {
        ...
        if let Ok(index) = self.get_index_store().await {
            ...
            // Index by type
            let _ = index.put(&source_key, agent_id_vec.clone().into()).await;
...
impl trust_core::traits::AgentRegistry for JetStreamAgentRegistry {
    ...
    async fn update(
        ...
        if let Some(kill_switch) = req.kill_switch {
            record.kill_switch = kill_switch;
        }
...
pub async fn bootstrap_from_toml_direct(js: &async_nats::jetstream::Context, path: &str) {
    let content = match std::fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            tracing::warn!(
                "⚠️ No agent bootstrap config at '{}': {} (agents can be added via API)",
                path,
                e
            );
            return;
    ...
    for entry in config.agents {
        ...
        let json = match serde_json::to_vec(&record) {
            ...
            Err(e) => {
                tracing::error!(
                    "❌ Failed to serialize bootstrap agent '{}': {}",
                    entry.agent_id,
                    e
                );
                continue;
            ...
            Ok(_) => {
                ...
                if let Some(ref idx) = index_store {
                    ...
                    let _ = idx
                        ...
                        .await;
                    let _ = idx
                        .put(&format!("id_{}", record.agent_id), aid_vec.into())
                        .await;
                }
            }
            Err(e) => {
