...
async fn ws_handler(
    ...
    let token = match token {
        Some(t) => t,
        None => {
            tracing::warn!("🚫 WebSocket connection attempt: missing token");
...
async fn handle_ws_socket(mut socket: WebSocket, state: Arc<GatewayState>, tenant_id: String) {
    ...
    loop {
        tokio::select! {
            maybe_ws_msg = socket.recv() => {
                match maybe_ws_msg {
                    Some(Ok(Message::Close(_))) | None => {
                        tracing::info!("🔌 WebSocket client disconnected for tenant {}", tenant_id);
                        break;
                    }
                    Some(Err(e)) => {
                        tracing::warn!("⚠️ WebSocket error for tenant {}: {}", tenant_id, e);
                        break;
                    }
            ...
            maybe_nats_msg = futures::StreamExt::next(&mut nats_sub) => {
                match maybe_nats_msg {
                    Some(nats_msg) => {
                        if let Ok(payload_str) = std::str::from_utf8(&nats_msg.payload) {
                            if let Err(e) = socket.send(Message::Text(payload_str.to_string())).await {
                                tracing::warn!("⚠️ Failed to send message to WS for tenant {}: {}", tenant_id, e);
                                break;
                            }
                        }
                    }
                    None => {
                        tracing::warn!("⚠️ NATS subscription ended for tenant {}", tenant_id);
                        break;
                    }
                }
            }
