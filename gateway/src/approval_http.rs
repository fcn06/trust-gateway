...
///
/// WS-H1: Returns `410 Gone` if the approval has expired (24h TTL).
pub async fn get_approval_handler(
    State(state): State<Arc<GatewayState>>,
    Path(approval_id): Path<String>,
) -> Result<Json<ApprovalRecord>, (StatusCode, Json<serde_json::Value>)> {
    match state.approval_store.get(&approval_id).await {
            ...
            if record.status == ApprovalStatus::Expired {
                return Err((
                    ...
                    Json(serde_json::json!({
                        ...
                        "message": "This approval has expired and can no longer be acted upon.",
        ...
        Ok(None) => Err((
            ...
            Json(serde_json::json!({
                "error": "not_found",
                "message": "Approval not found. It may have expired (24h TTL).",
                "approval_id": approval_id,
            })),
        )),
        Err(e) => {
            ...
            let requests = records
                ...
                .map(|r| {
                    EscalationRequest {
                        ...
                        owner_user_id: Some(verified.requester_did.clone()),
...
pub async fn approve_escalation_handler(
    ...
    let decision = DecisionRequest {
        ...
        actor_did: Some(verified.requester_did.clone()),
        reason: None,
        correlation_id: None,
    };
...
async fn process_decision(
    ...
    let new_status = match body.decision.as_str() {
        ...
        "denied" | "deny" => {
            state
                .approval_store
                .mark_denied(&approval_id, result)
                .await
                .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            ApprovalStatus::Denied
        }
    ...
    if let Err(e) = state
        .nats
        .publish(result_subject.to_string(), payload.to_string().into())
        .await
    {
                    ...
                    if let Ok(record) = serde_json::from_slice::<ApprovalRecord>(&entry) {
                        if record.action_id == action_id {
                            let status = match record.status {
                                ApprovalStatus::Pending | ApprovalStatus::PendingProof => {
                                    "pending_approval"
                                }
