...
pub async fn spawn_projector(
    ...
    let kv_store = match js.get_key_value("action_timelines").await {
        ...
        Err(e) => {
            tracing::warn!(
                "⚠️ Cannot access action_timelines KV bucket (projector disabled): {}",
                e
            );
    ...
    let stream = match js.get_stream(audit_stream_name).await {
        ...
        Err(e) => {
            tracing::warn!(
                "⚠️ Cannot access AUDIT_EVENTS stream (projector disabled): {}",
                e
            );
            return Ok(tokio::spawn(async {}));
        }
...
async fn process_audit_message(
    ...
    let mut timeline = match kv_store.get(&action_id).await {
        Ok(Some(entry)) => serde_json::from_slice::<ActionTimeline>(&entry)
    ...
    tracing::debug!(
        "📝 Timeline processed for action {} (event: {}, seq: {}, status: {})",
        action_id,
        event_type_str,
        stream_seq,
...
mod tests {
    ...
    fn test_event_labels() {
        ...
        assert_eq!(
            event_label("approval_approved", &empty),
            "Approved by human"
        );
    ...
    fn test_event_label_auth_denial_vs_execution_failure() {
        ...
        assert_eq!(
            event_label("action_failed", &auth_details),
            "Authentication denied"
        );
    ...
    fn test_projected_status() {
        ...
        assert_eq!(
            projected_status("approval_requested", &serde_json::json!({})),
            "waiting_approval"
        );
        assert_eq!(
            projected_status("approval_approved", &serde_json::json!({})),
            "approved"
        );
        assert_eq!(
            projected_status("approval_denied", &serde_json::json!({})),
            "denied"
        );
        assert_eq!(
            projected_status("action_succeeded", &serde_json::json!({})),
            "executed"
        );
        assert_eq!(
            projected_status("action_retried", &serde_json::json!({})),
            "retrying"
        );
        assert_eq!(
            projected_status("proof_verified", &serde_json::json!({"verified": true})),
            "approved"
        );
        assert_eq!(
            projected_status("proof_verified", &serde_json::json!({"verified": false})),
            "denied"
        );
    }
    ...
    fn test_projected_status_auth_denial_vs_execution_failure() {
        ...
        assert_eq!(
            projected_status(
                "action_failed",
                &serde_json::json!({"stage": "authentication"})
            ),
            "denied"
        );
        assert_eq!(
            projected_status("action_failed", &serde_json::json!({"stage": "execution"})),
            "failed"
        );
        assert_eq!(
            projected_status("action_failed", &serde_json::json!({})),
            "failed"
        );
    }
}
