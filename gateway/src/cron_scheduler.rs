                ...
                if let Some(cron_expr) = &entry.cron {
                    ...
                    match Schedule::from_str(cron_expr) {
                        Ok(schedule) => {
                            ...
                            let ref_time = check_time - chrono::Duration::seconds(5);
                            let mut upcoming = schedule.after(&ref_time);
                        ...
                        Err(e) => {
                            tracing::warn!(
                                "⚠️ Invalid cron expression '{}' for tool '{}': {}",
                                cron_expr,
                                tool_name,
                                e
                            );
...
async fn dispatch_cron_action(
    state: &Arc<GatewayState>,
    tool_name: &str,
    _entry: &crate::router::ToolRegistryEntry,
) {
