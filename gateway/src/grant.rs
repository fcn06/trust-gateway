...
impl Ed25519GrantIssuer {
    ...
    pub fn issue(
        ...
        // REC-3: JWT class separation — aud="executor-host" prevents session JWT substitution
        let claims = Claims::with_custom_claims(grant.clone(), Duration::from_secs(ttl.as_secs()))
            .with_issuer("trust_gateway")
            .with_subject(&req.action_id)
            .with_jwt_id(&grant_id)
            .with_audience("executor-host");
...
#[deprecated(note = "SEC-1: Use Ed25519GrantIssuer in production. HMAC is for development only.")]
...
mod tests {
    ...
    fn ed25519_grant_round_trip() {
        ...
        let signed = issuer
            .issue(&req, GrantClearance::AutoApproved, ttl)
        ...
        let verified = pub_key
            .verify_token::<ExecutionGrant>(&signed.token, Some(options))
        ...
        assert_eq!(
            verified.custom.allowed_action,
    ...
    fn ed25519_grant_rejects_tampered_token() {
        ...
        let signed = issuer
            .issue(&req, GrantClearance::HumanApproved, ttl)
            .expect("Ed25519 issue should succeed");
    ...
    fn ed25519_grant_different_key_rejects() {
        ...
        assert!(
            result.is_err(),
            "Ed25519 grant verified with wrong key must be rejected"
        );
    }
    ...
    fn ed25519_grant_contains_correct_claims() {
        ...
        let signed = issuer
            .issue(&req, GrantClearance::ProofVerified, ttl)
            .expect("Ed25519 issue should succeed");
    ...
    fn ed25519_pem_round_trip() {
        ...
        assert!(
            result.is_ok(),
            "Token from restored PEM key must verify with original public key"
        );
    }
    ...
    fn hmac_grant_rejects_wrong_key() {
        ...
        assert!(
            result.is_err(),
            "HMAC grant validated with wrong key must be rejected"
        );
    }
    ...
    // ── SEC-2: input_hash Tests (now mandatory String) ───────

    #[test]
    fn ed25519_grant_contains_input_hash() {
        let issuer = Ed25519GrantIssuer::generate("kid-hash".into());
        let req = test_action_request();
        let ttl = std::time::Duration::from_secs(30);
    ...
    #[test]
    fn hmac_grant_contains_input_hash() {
        let issuer = HmacGrantIssuer::new("hash-test-secret");
        let req = test_action_request();
        let ttl = std::time::Duration::from_secs(30);
    ...
    fn input_hash_is_deterministic() {
        let issuer = Ed25519GrantIssuer::generate("kid-det".into());
        let req = test_action_request();
        let ttl = std::time::Duration::from_secs(30);
        ...
        assert_eq!(
            signed1.claims.input_hash, signed2.claims.input_hash,
            "Same arguments must produce the same input_hash"
        );
    }

    #[test]
    fn input_hash_changes_with_different_args() {
        let issuer = Ed25519GrantIssuer::generate("kid-diff".into());
        let ttl = std::time::Duration::from_secs(30);

        let mut req1 = test_action_request();
        req1.action.arguments = serde_json::json!({"summary": "Event A"});

        let mut req2 = test_action_request();
        req2.action.arguments = serde_json::json!({"summary": "Event B"});

        let signed1 = issuer
            .issue(&req1, GrantClearance::AutoApproved, ttl)
            .unwrap();
        let signed2 = issuer
            .issue(&req2, GrantClearance::AutoApproved, ttl)
            .unwrap();

        assert_ne!(
            signed1.claims.input_hash, signed2.claims.input_hash,
            "Different arguments must produce different input_hash values"
        );
    }

    #[test]
    fn input_hash_round_trip_through_jwt() {
        let issuer = Ed25519GrantIssuer::generate("kid-rt".into());
        let req = test_action_request();
        let ttl = std::time::Duration::from_secs(30);
        ...
        let signed = issuer
            .issue(&req, GrantClearance::AutoApproved, ttl)
            .unwrap();
        let expected_hash = signed.claims.input_hash.clone();

        // Verify the JWT and check input_hash survives round-trip
        let pub_key = issuer.key_pair.public_key();
        let verified = pub_key
            .verify_token::<ExecutionGrant>(&signed.token, None)
            .expect("Verification should succeed");

        assert_eq!(
            verified.custom.input_hash, expected_hash,
            "input_hash must survive JWT encode/decode round-trip"
        );
    }
}
