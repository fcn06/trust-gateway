...
async fn main() -> Result<()> {
    ...
    let app_registry_kv = match js.get_key_value("app_registry").await {
        Ok(store) => Some(store),
        Err(e) => {
            tracing::warn!("⚠️ Failed to load app_registry KV bucket: {}", e);
            None
        }
    };

    let app_registry = Arc::new(crate::app_registry::AppRegistry::new(
        app_registry_kv,
        tool_registry.clone(),
    ...
    // Restore registered apps from KV in background
    let restore_app_registry = app_registry.clone();
    ...
    let app_reg_state = state.clone();
    let app_reg_token = cancel_token.clone();
    background_tasks.spawn(run_supervised(
        "app_registry_listener",
        app_reg_token,
        Some(state.task_statuses.clone()),
        move || {
            let state = app_reg_state.clone();
            async move {
                if let Some(ref app_reg) = state.app_registry {
                    app_reg.listen_for_registrations().await
                } else {
                    Ok(())
                }
            }
