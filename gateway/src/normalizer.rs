...
use trust_core::{
    ActionDecision, ActionRequest, ActionReview, BusinessDiffField, NormalizedAction,
    PolicyEvaluationDisplay, RiskLevel,
};
...
pub fn normalize_action(action_name: &str, args: &serde_json::Value) -> NormalizedAction {
    ...
    if action_name.starts_with("shopify_order_refund")
        || action_name.starts_with("shopify.order.refund")
    {
    ...
    } else if action_name.starts_with("google_calendar")
        || action_name.starts_with("google.calendar")
    {
...
pub fn compute_risk_level(decision: &ActionDecision) -> RiskLevel {
    match decision {
        ...
        ActionDecision::RequireApproval { tier, .. } => match tier {
...
pub fn build_policy_display(decision: &ActionDecision, reason: &str) -> PolicyEvaluationDisplay {
    match decision {
        ...
        ActionDecision::RequireApproval {
            ...
            let effect = match tier {
                ...
                trust_core::ApprovalTier::Tier1PortalClick => {
                    "Requires a single click approval in the portal".to_string()
                }
                trust_core::ApprovalTier::Tier2ReAuthenticate => {
                    "Requires re-authentication before approval".to_string()
                }
                trust_core::ApprovalTier::Tier3VerifiedPresentation => {
                    "Requires a verified credential presentation".to_string()
                }
            };
            PolicyEvaluationDisplay {
...
pub fn build_action_review(
    ...
    let proof_request_val = match decision {
        ActionDecision::RequireProof { proof_request, .. } => {
            serde_json::to_value(proof_request).ok()
        }
        _ => None,
    };
    ...
    ActionReview {
        ...
        timeout_seconds: 120,
        execution_preview:
            "If approved, a one-time execution grant valid for 30 seconds will be issued"
                .to_string(),
        proof_request: proof_request_val,
        approved_by: None,
...
mod tests {
    ...
    fn test_shopify_refund_normalizer() {
        ...
        assert_eq!(
            result.change_preview[1].after,
            Some("620.00 EUR".to_string())
        );
    ...
    fn test_risk_level_from_decision() {
        let allow = ActionDecision::Allow {
            policy_id: "p1".to_string(),
        };
        assert_eq!(compute_risk_level(&allow), RiskLevel::Low);

        let deny = ActionDecision::Deny {
