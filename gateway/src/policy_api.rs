...
pub async fn list_rules_handler(State(state): State<Arc<GatewayState>>) -> Json<serde_json::Value> {
...
pub async fn create_rule_handler(
    ...
    tracing::info!(
        "📜 Policy rule creation requested: {} → {:?}",
        rule_id,
        rule_json
    );
...
pub async fn simulate_handler(
    ...
    let operation = match req.operation_kind.as_deref() {
        ...
        _ => infer_operation(&req.action_name),
    };

    let category = infer_category(&req.action_name);
