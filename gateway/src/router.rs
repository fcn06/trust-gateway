...
enum ExecutorTarget {
    ...
    VpMcp,
    InternalMeta,
    /// Dynamically registered transport (app_id references the TransportRegistry).
    DynamicTransport(String),
}
...
pub struct ToolRegistryEntry {
    ...
    /// Optional app_id for dynamically registered tools (from AppManifest).
...
impl ToolRegistry {
    ...
    /// Register tools dynamically at runtime (hot-plug from AppRegistry).
    ///
...
async fn determine_executor_target(state: &GatewayState, tool_name: &str) -> ExecutorTarget {
    ...
    // Phase 1.5: Check dynamic transport registry by tool name.
    if let Some(ref transport_registry) = state.transport_registry {
        if let Some((app_id, _)) = transport_registry.find_by_tool_name(tool_name).await {
            tracing::info!(
                "📋 Transport registry routing: '{}' → app='{}'",
                tool_name,
                app_id
            );
            return ExecutorTarget::DynamicTransport(app_id);
        }
    }
...
pub async fn dispatch_to_connector(
    ...
    let connector_key = match &target {
        ...
        ExecutorTarget::DynamicTransport(_) => "dynamic_transport",
    };
...
async fn dispatch_to_dynamic_transport(
    state: &GatewayState,
    req: &ActionRequest,
    grant: &SignedGrant,
    app_id: &str,
    ...
    let dispatch_req = trust_core::transport::TransportDispatchRequest {
        tool_name: req.action.name.clone(),
        arguments: req.action.arguments.clone(),
        grant_jwt: grant.token.clone(),
        input_hash,
    ...
    let result = transport.dispatch(&dispatch_req).await?;
