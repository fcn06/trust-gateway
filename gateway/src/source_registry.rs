...
use identity_context::source::{AuthMode, SourceRegistration, SourceType};
...
fn default_source_type() -> String {
    "external_swarm".to_string()
}
fn default_policy_profile() -> String {
    "external_default".to_string()
}
...
pub fn load_sources_from_env() -> Vec<SourceRegistration> {
    ...
    let registrations: Vec<SourceRegistration> = config
        .sources
        .into_iter()
...
"#;
        ...
        let regs: Vec<SourceRegistration> = config
            .sources
            .into_iter()
