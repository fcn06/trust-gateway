...
pub async fn webhook_post_handler(
    ...
    tracing::info!(
        "📥 Received webhook from provider: {} (Event: {})",
        provider,
        event_id
    );
    ...
    if !is_valid {
        tracing::warn!(
            "❌ Webhook signature validation failed for provider: {}",
            provider
        );
    ...
    match state
        .nats
        .publish(subject, serde_json::to_vec(&message).unwrap().into())
        .await
    {
        Ok(_) => {
