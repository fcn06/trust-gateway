...
use x25519_dalek::{PublicKey, StaticSecret}; // Fix: Import StreamExt
...
async fn test_jit_routing_e2e() -> anyhow::Result<()> {
    ...
    let gateway_priv_b64 =
        base64::engine::general_purpose::STANDARD.encode(gateway_secret.to_bytes());

    // 2. Start Gateway
    // We assume public_gateway binary is buildable.
    println!("🚀 Starting Gateway...");
    let mut child = Command::new(env!("CARGO_BIN_EXE_public_gateway"))
    ...
    let resp = client
        .post("http://localhost:3009/ingress")
...
#[tokio::test]
async fn test_security_path_filtering() -> anyhow::Result<()> {
    // 1. Setup Gateway Key
    let gateway_secret = StaticSecret::random_from_rng(rand::thread_rng());
    let gateway_priv_b64 =
        base64::engine::general_purpose::STANDARD.encode(gateway_secret.to_bytes());
    ...
    // 2. Start Gateway
    println!("🚀 Starting Gateway for security test...");
    let mut child = Command::new(env!("CARGO_BIN_EXE_public_gateway"))
        .env("GATEWAY_PRIVATE_KEY", &gateway_priv_b64)
        .env("NATS_URL", "nats://localhost:4222")
        .env("PORT", "3011") // Use different port
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()?;
    ...
    let resp = client.get("http://localhost:3011/health").send().await?;
    assert_eq!(resp.status(), reqwest::StatusCode::OK);

    // Verify malicious/suspicious paths are blocked (returning 403 Forbidden)
    let bad_paths = [
        "/.env",
        "/.git/config",
        "/wp-admin",
    ...
    for path in &bad_paths {
        let url = format!("http://localhost:3011{path}");
        let resp = client.get(&url).send().await?;
        assert_eq!(
            resp.status(),
            reqwest::StatusCode::FORBIDDEN,
            "Path {path} was not blocked!"
        );
    }
