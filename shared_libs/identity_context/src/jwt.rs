...
///
/// Wraps `jwt_simple::prelude::Ed25519PublicKey` to provide the `AuthVerifier` contract.
pub struct Ed25519AuthVerifier {
    public_key: jwt_simple::prelude::Ed25519PublicKey,
    allowed_audience: Option<String>,
    required_issuer: Option<String>,
}

impl Ed25519AuthVerifier {
...
impl AuthVerifier for Ed25519AuthVerifier {
    fn verify(&self, token: &str) -> Result<VerifiedJwt, AuthVerifyError> {
        if token.is_empty() {
            return Err(AuthVerifyError::MissingToken);
        }
...
mod tests {
    ...
    fn test_backward_compat_extract_jti() {
        ...
        assert_eq!(extract_jti_from_jwt(&jwt), Some("session-xyz".to_string()));
    }
