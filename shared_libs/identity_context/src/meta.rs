...
pub fn extract_meta(args: &mut serde_json::Value) -> Result<MetaPayload, MetaError> {
    ...
    let ag_block = meta_value
        .get("io.lianxi")
        .or_else(|| meta_value.get("io").and_then(|io| io.get("lianxi")))
...
pub fn build_identity_context_verified(
    ...
    Ok(crate::models::IdentityContext {
        ...
        owner_did: verified_claims
            .user_did
            .clone()
            .unwrap_or_else(|| verified_claims.iss.clone()),
        requester_did,
        session_jwt: meta.session_jwt.clone(),
        auth_level: Default::default(),
        auth_method: Default::default(),
        oauth_scopes: verified_claims.scope.clone(),
        source,
    })
}
...
mod tests {
    ...
    fn test_extract_meta_success() {
        ...
        assert!(
            args.get("_meta").is_none(),
            "_meta must be removed from args"
        );
        ...
        let identity =
            build_identity_context(&meta, Default::default()).expect("should build identity");
    ...
    fn test_strip_meta_guarantees_removal() {
        ...
        assert!(
            args.get("_meta").is_none(),
            "_meta MUST be absent after strip"
        );
