...
pub enum AuditEventType {
    ...
    // ── Security events ──
    /// An egress validation rule caught sensitive data attempting to leave the boundary.
    EgressViolation,

    /// An ExecutionGrant JWT was replayed (same JTI presented twice).
    GrantReplayBlocked,
    /// Policy file was loaded/reloaded at gateway startup.
    PolicyLoaded,
}
...
impl std::fmt::Display for AuditEventType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            Self::EgressViolation => "egress.violation",
