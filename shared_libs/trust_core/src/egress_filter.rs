...
/// Structured DLP field classification tag
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FieldSecurityClass {
    PiiContact,
    InternalIdentifier,
...
/// Pre-execution result handling constraints for execution output validation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ResultHandlingPolicy {
    /// Maximum allowed payload bytes in executor result (e.g. 1MB default)
    pub max_result_bytes: usize,
    /// Data sensitivity classes allowed in output
    pub allowed_data_classes: Vec<FieldSecurityClass>,
    /// Redaction profile: "strict" | "standard" | "none"
    pub redaction_profile: String,
}

impl Default for ResultHandlingPolicy {
    fn default() -> Self {
        Self {
...
impl StructuredFieldClassifier {
    ...
    pub fn filter_structured_json(
        json: &mut serde_json::Value,
        field_schema: &std::collections::HashMap<String, FieldSecurityClass>,
        allowed_audience: &str,
    ) {
        if let serde_json::Value::Object(map) = json {
            for (key, class) in field_schema {
