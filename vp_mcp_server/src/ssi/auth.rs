...
async fn validate_vp_signature(vp: &str, sender_did: &str) -> std::result::Result<bool, String> {
    ...
    let verified = trust_auth::did::verify_presentation(vp, &client, None).await
        .map_err(|e| format!("VP signature verification failed: {}", e))?;
    
    if verified.agent_did != sender_did {
        return Err(format!("Binding mismatch: VP agent_did '{}' does not match sender_did '{}'", verified.agent_did, sender_did));
    }
...
fn extract_public_key(sender_did_str: &str) -> std::result::Result<Vec<u8>, String> {
    if let Some(key_raw) = sender_did_str.strip_prefix("did:twin:") {
        if key_raw.starts_with('z') {
            if let Ok(decoded) = bs58::decode(key_raw).into_vec() {
                ...
                if let Ok(decoded) = hex::decode(&key_raw[1..]) {
                    if decoded.len() == 32 {
                        tracing::warn!(
                            "Detected Hex encoded key with 'z' prefix. Accepting as legacy/quirk."
                        );
                        return Ok(decoded);
                    } else {
                    ...
                    return Err(
                        "Invalid or unsupported key in did:twin Multibase (tried Base58 and Hex)"
                            .to_string(),
                    );
                }
            }
        ...
        let decoded = bs58::decode(key_multibase)
            .into_vec()
            .map_err(|e| format!("Invalid Base58 encoding: {}", e))?;
        if decoded.len() != 34 {
            return Err(format!(
                "Invalid did:key length: {}. Expected 34 bytes for Ed25519",
                decoded.len()
            ));
        }
    ...
    Err(format!(
        "Unsupported DID method: {}. Expected did:twin or did:key",
        sender_did_str
    ))
}
...
pub async fn verify_session_jwt(jwt: &str) -> std::result::Result<JwtClaims, String> {
    match trust_auth::did::verify_eddsa_session_jwt(jwt) {
        Ok(claims) => {
            let user_did = claims.user_did.unwrap_or_else(|| claims.iss.clone());
            let clearance_level = claims.scope.iter()
                .find_map(|s| s.strip_prefix("clearance:").map(|v| v.to_string()))
                .unwrap_or_else(|| "standard".to_string());
            
            Ok(JwtClaims {
                sender_did: claims.iss,
                user_did,
                clearance_level,
                scope: claims.scope,
            })
